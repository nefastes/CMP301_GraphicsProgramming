...
BloomCompute::BloomCompute(ID3D11Device* device, HWND hwnd, int screen_width, int screen_height) : BaseShader(device, hwnd),
output_texture(nullptr), srv(nullptr), uav(nullptr)
{
	initShader(L"bloom_cs.cso", screen_width, screen_height);
}

BloomCompute::~BloomCompute()
{
}
...
void BloomCompute::initShader(const wchar_t* filename, int screen_width, int screen_height)
{
