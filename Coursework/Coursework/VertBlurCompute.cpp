...
VertBlurCompute::VertBlurCompute(ID3D11Device* device, HWND hwnd, int screen_width, int screen_height) : BaseShader(device, hwnd),
output_texture(nullptr), srv(nullptr), uav(nullptr)
{
	initShader(L"blur_vertical_cs.cso", screen_width, screen_height);
}

VertBlurCompute::~VertBlurCompute()
{
}

void VertBlurCompute::setShaderParameters(ID3D11DeviceContext* device_context, ID3D11ShaderResourceView* buffer_to_blur)
{
...
void VertBlurCompute::unbind(ID3D11DeviceContext* device_context)
{
...
void VertBlurCompute::initShader(const wchar_t* filename, int screen_width, int screen_height)
{
