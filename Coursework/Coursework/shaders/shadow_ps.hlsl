...
cbuffer LightBuffer : register(b0)
{
    ...
    float4 falloff_spotAngle_padding_padding[N_LIGHTS];
};
...
float4 main(InputType input) : SV_TARGET
{
    ...
    for (int i = 0, mapID = 0; i < N_LIGHTS; ++i, mapID += 6)
    {
        ...
        float spot_falloff = falloff_spotAngle_padding_padding[i].x;
        float spot_angle = falloff_spotAngle_padding_padding[i].y;
