...
BloomThresholdCompute::BloomThresholdCompute(ID3D11Device* device, HWND hwnd, int screen_width, int screen_height) : BaseShader(device, hwnd),
output_texture(nullptr), srv(nullptr), uav(nullptr)
{
	initShader(L"bloom_threshold_cs.cso", screen_width, screen_height);
}

BloomThresholdCompute::~BloomThresholdCompute()
{
}

void BloomThresholdCompute::setShaderParameters(ID3D11DeviceContext* device_context, ID3D11ShaderResourceView* buffer_to_threshold, float threshold_value)
{
	...
	device_context->CSSetShaderResources(0, 1, &buffer_to_threshold);
	//Send the UAV to the compute shader so we can write the output to it
	device_context->CSSetUnorderedAccessViews(0, 1, &uav, 0);
}

void BloomThresholdCompute::unbind(ID3D11DeviceContext* device_context)
{
...
void BloomThresholdCompute::initShader(const wchar_t* filename, int screen_width, int screen_height)
{
