...
BloomCombineCompute::BloomCombineCompute(ID3D11Device* device, HWND hwnd, int screen_width, int screen_height) : BaseShader(device, hwnd),
output_texture(nullptr), srv(nullptr), uav(nullptr)
{
	initShader(L"bloom_combine_cs.cso", screen_width, screen_height);
}

BloomCombineCompute::~BloomCombineCompute()
{
}

void BloomCombineCompute::setShaderParameters(ID3D11DeviceContext* device_context, ID3D11ShaderResourceView* buffer_to_combine_1, ID3D11ShaderResourceView* buffer_to_combine_2)
{
	//Send the buffer to be blurred to the compute shader
	device_context->CSSetShaderResources(0, 1, &buffer_to_combine_1);
	device_context->CSSetShaderResources(1, 1, &buffer_to_combine_2);
	//Send the UAV to the compute shader so we can write the output to it
	device_context->CSSetUnorderedAccessViews(0, 1, &uav, 0);
}

void BloomCombineCompute::unbind(ID3D11DeviceContext* device_context)
{
...
void BloomCombineCompute::initShader(const wchar_t* filename, int screen_width, int screen_height)
{
